	...
	window.jvfrm_spot_map_box_func = {
		...
		, setMarkers: function( response )
		{
			...
			if( item_markers.length > 0 )
			{
				...
				if( init_mapZoom > 0 && !this.first_set_marker ) {
					_opt.map				={
						options:{ zoom : init_mapZoom }
					}
					this.el.gmap3( _opt );
					this.first_set_marker		= true;
					return;
				}
		...
		, map_control : function( block ){
			var
				...
				distanceBar	= $( '.javo-geoloc-slider', container ),
				input				= $( 'input', container ),
				dropdown	= $( 'select', container ),
				buttons		= $( 'button', container ),
				toggleBtn		= $( '.javo-map-box-filter-items span', container ),
				geoButton	= $( '.javo-my-position', container ),
				elements		= new Array(
					distanceBar,
					input,
					toggleBtn,
		...
		, moduleLink : function() {
			...
			$( document ).on ( 'hover', '.module[data-post-id]', function(){

				var currentModule	= $( this ).data( 'post-id' );
				...
				obj.swapIcon( currentModule, _marker, 9999 );
		...
		, getStyles : function() {
			var primary_color = obj.args.map_primary_color;
			if( !primary_color ){
				return false;
			}
			return [
...
jvfrm_spot_multi_listing.prototype = {
	...
	mobileFilter : function() {
		...
		var
			...
			minMoved = false,
			maxMoved = false,
			filter_wrap = $( '.javo-maps-search-wrap' ),
