...
class Jv_WPML_func
{
	...
	{
		add_action( 'jvfrm_spot_wpml_switcher'		, Array( __CLASS__, 'switcher' ) );
	...
	public static function switcher()
	{
		...
		if( false === (boolean)( $jvfrm_spot_wpml_langs = icl_get_languages('skip_missing=0') ) )
			return;

		if( false === (boolean)( $jvfrm_spot_cur_flag_url = $jvfrm_spot_wpml_langs[ ICL_LANGUAGE_CODE ]['country_flag_url'] ) )
			return;
		...
		<div id="javo-wpml-switcher" class="inline-block">
			<button class="btn btn-primary admin-color-setting">
				<img src="<?php echo esc_attr( $jvfrm_spot_cur_flag_url );?>">
			</button>
			<div>
				<ul class="javo-wpml-switcher-languages">
					<?php
					if( !empty( $jvfrm_spot_wpml_langs ) )
						foreach( $jvfrm_spot_wpml_langs as $lang )
						{
