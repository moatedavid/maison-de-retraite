<?php
function jvfrm_spot_get_demoStatus(){
	global $wpdb;
	return $wpdb->get_var(
		$wpdb->prepare(
			"select ID from $wpdb->posts where post_type='%s' and post_status='%s'",
			jvfrm_spot_core()->slug,
			'publish'
		)
	);
}
...
$arrGuideSections	= Array(
	'plugin-actived'		=> Array(
		'label'				=> esc_html__( "Active Requested Plugins", 'javospot' )
		, 'active'			=> true
		, 'passed'			=>
		class_exists( 'Javo_Spot_Core' ) &&
		class_exists( 'Lava_Directory_Manager' ) &&
		class_exists( 'Vc_Manager' ) &&
		class_exists( 'Ultimate_VC_Addons' )
		, 'plugins'									=> Array(
			'Javo_Spot_Core'						=> esc_html__( "Javo Spot Core", 'javospot' )
			, 'Lava_Directory_Manager'		=> esc_html__( "Lava Directory Manager", 'javospot' )
			, 'Vc_Manager'						=> esc_html__( "WPBakery Visual Composer", 'javospot' )
			, 'Ultimate_VC_Addons'			=> esc_html__( "Ultimate Addons for Visual Composer", 'javospot' )
		)
	)
	, 'demo-installed'	=> Array(
		'label'				=> esc_html__( "Demo Installation", 'javospot' )
		, 'passed'			=> jvfrm_spot_get_demoStatus()
	)
	, 'permalink'			=> Array(
		'label'				=> esc_html__( "Permalink Setup", 'javospot' )
		, 'passed'			=> strstr( get_option( 'permalink_structure' ), 'postname' )
	)
	, 'widget-installed'	=> Array(
		'label'				=> esc_html__( "Widget Installation", 'javospot' )
		, 'passed'			=> is_active_sidebar( 'lava-lv_listing-single-sidebar' )
	)
	, 'menu-installed'	=> Array(
		'label'				=> esc_html__( "Menu Setup", 'javospot' )
		, 'passed'			=> has_nav_menu( 'primary' )
	)
	, 'static-installed'	=> Array(
		'label'				=> esc_html__( "Front Page Setup", 'javospot' )
		, 'passed'			=> get_option( 'page_on_front' )
	)
	, 'setting-updated'	=> Array(
		'label'				=> esc_html__( "Theme Settings Setup", 'javospot' )
		, 'passed'			=> get_option( 'jvfrm_spot_themes_settings' )
	)
); ?>

<div class="about-wrap jv-admin-wrap">
    <h1><?php echo$objTheme->name; ?> <?php esc_html_e( "Default Setting / System Status", 'javospot' ); ?></h1>
    <div class="about-text" style="margin-bottom: 32px;">

        <p>
			<?php esc_html_e( "Please have a look at default settings and system status. default settings is for you should complete (100%) to use our theme properly.", 'javospot' ); ?>
        </p>
    </div>

	<div class=" jv-default-setting-status-wrap">
		<h3 class="jv-default-setting-status-title">
			<?php esc_html_e( "Default Settings Status", 'javospot' ); ?>
			<div class="jv-default-setting-status-progress">
				<span><?php esc_html_e( "Loading", 'javospot' ); ?></span> <?php esc_html_e( "Completed", 'javospot' ); ?>
			</div>
		</h3>
		<div><?php esc_html_e( "These default settings should be installed or actived to make this theme work properly.", 'javospot' ); ?></div>
		<?php
		if( !empty( $arrGuideSections ) ) : foreach( $arrGuideSections as $section => $sectionMeta ) {
			$strFileName	= JVFRM_SPOT_ADM_DIR . "/templates/step-{$section}.php";
			$isComplete		= $sectionMeta[ 'passed' ] ? esc_html__( "Completed", 'javospot' ) : esc_html__( "Incompleted", 'javospot' );
