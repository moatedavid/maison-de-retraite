;(function($){
	"use strict";
	jQuery.jvfrm_spot_mail = function(d, a, b){
		d = $.extend( true, {}, {
			url:null
			, subejct: null
			, to: null
		...
		options.error = function(e){
			$.jvfrm_spot_msg({ content: 'Error : ' + e.state() });
			console.log( e.responseText );
		};
		options.success = function(data){
			if(data.result){
				$.jvfrm_spot_msg({ content:d.successMsg });
				if( typeof d.callback == "function" ) d.callback();
			}else{
				$.jvfrm_spot_msg({ content:d.failMsg });
			};
