<?php
/**
 * The gallery format template for displaying content
 *
...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="jv-single-post-header">
		...
		<section class="jv-single-post-meta">
			<ul class="list-inline">
				<li class="author-name"><?php echo esc_html( $jvfrm_spot_author->display_name ); ?></li><li>/</li>
				...
				<li class="comments">/</li><li>
					...
					<?php
					comments_popup_link(
						esc_html__( '0 Comment', 'javospot' )
						, esc_html__( '1 Comment', 'javospot' )
	...
	<section class="jv-single-post-contents">
		<div class="entry-content">
			<?php
			the_content( sprintf(
				wp_kses(__( 'Continue reading %s <span class="meta-nav">&rarr;</span>', 'javospot' ), jvfrm_spot_allow_tags() ),
				the_title( '<span class="screen-reader-text">', '</span>', false )
			) );

			wp_link_pages( array( 'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'javospot' ), 'after' => '</div>' ) );
			?>
		</div><!-- .entry-content -->
	...
	<footer class="jv-single-post-footer">
		...
		<?php
		edit_post_link(
			sprintf( "<i class=\"fa fa-cog\"></i> %s", esc_html__( 'Edit', 'javospot' ) ),
			...
			<div id="jv-single-post-author-info" class="media">
				<div class="media-body text-right">
					<a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author">
						<h4 class="media-heading"><?php printf( wp_kses(__( 'About <em>%s</em>', 'javospot'), jvfrm_spot_allow_tags() ), get_the_author() ); ?></h4>
						<p><?php echo wp_kses( get_the_author_meta( 'description' ), jvfrm_spot_allow_tags() ); ?></p>
					</a>
				</div><!-- /.media-body -->
				<div class="media-right">
					<a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author">
						...
						$author_bio_avatar_size = apply_filters( 'jvfrm_spot_drt_author_bio_avatar_size', 68 );
