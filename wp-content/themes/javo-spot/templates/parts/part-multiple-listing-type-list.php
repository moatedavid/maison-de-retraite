<?php
$arrListingFilters	= apply_filters( 'jvfrm_spot_' . $post->lava_type . '_map_list_filters', Array() );
$strListOutputClass = sprintf(
	'class="%s"', join(
		' ',
		apply_filters(
			'jvfrm_spot_map_list_output_class',
			Array( 'javo-shortcode' )
		)
	)
); ?>
<div id="map-list-style-wrap" <?php jvfrm_spot_map_class( 'container' ); ?>>
	<?php do_action( 'jvfrm_spot_' . $post->lava_type . '_map_list_wrap_before', $post ); ?>
	<div class="row">
											...
											if( isset( $element[ 'taxonomy' ] ) && taxonomy_exists( $element[ 'taxonomy' ] ) ) {
												$arrJavoOutput[]	= "
													...
													$arrJavoOutput[]	= apply_filters(
														'jvfrm_spot_get_selbox_child_term_lists'
														, $element['taxonomy']
														, null
														, 'select'
														, $element[ 'value' ]
											...
											if(
												...
												$arrJavoOutput[]			= "<div class=\"checkbox\">";
													$arrJavoOutput[]		= "<label>";
														...
														$arrJavoOutput[]	= "<input type=\"checkbox\" name=\"jvfrm_spot_list_multiple_filter\" value=\"{$term->term_id}\"" . ' ';
		...
		<div class="<?php echo apply_filters( 'jvfrm_spot_' . $post->lava_type . '_map_list_content_column_class' , 'col-sm-9', $post ) ; ?>">
			<div class="row">
				<div id="results" class="col-md-12 col-xs-12">
					<div id="spaces" class="col-md-12 col-xs-12">
						<?php do_action( 'jvfrm_spot_' . $post->lava_type . '_map_list_container_before', $post ); ?>
					...
					<?php do_action( 'jvfrm_spot_' . $post->lava_type . '_map_list_container_after', $post ); ?>
				</div><!--/#results-->
			</div><!--/.row-->
		</div><!-- /.col-md-9 -->
	</div><!-- /.row -->
	<?php do_action( 'jvfrm_spot_' . $post->lava_type . '_map_list_wrap_after', $post ); ?>
</div><!-- /.container-->