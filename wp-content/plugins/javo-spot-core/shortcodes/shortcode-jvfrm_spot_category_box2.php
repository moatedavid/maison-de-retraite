<?php
add_action( 'after_setup_theme', 'register_jvfrm_spot_category_box2' );
function register_jvfrm_spot_category_box2() {
	$objInstance = new jvfrm_spot_category_box2;
	add_shortcode( 'jvfrm_spot_category_box2', Array( $objInstance, 'output' ) );
}
class jvfrm_spot_category_box2 extends JvfrmSpot_ShortcodeParse
{
	...
	public function rander()
	{
		...
		if( ! empty( $objTerms ) ) : foreach( $objTerms as $term_id =>$term_name ) {
			...
			$termFeaturedMeta	= wp_get_attachment_image( $termFeaturedID, 'jvfrm-spot-avatar', false, Array( 'class' => 'img-responsive' ) );
						...
						'<div class="module">',
							'<div class="jv-thumb thumb-wrap">',
								...
								sprintf(
									...
									$termHasPosts,
									_n( "Post", "Posts", $termHasPosts, 'javo' )
								),
							'</div>',
