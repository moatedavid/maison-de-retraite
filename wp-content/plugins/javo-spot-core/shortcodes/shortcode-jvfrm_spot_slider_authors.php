<?php
class jvfrm_spot_slider_authors
{
	static $load_script = false;
	public function __construct()
	{
		add_shortcode( 'jvfrm_spot_slider_authors', Array( __CLASS__, 'jvfrm_spot_inline_category_slider_callback'));
		add_action( 'wp_footer', Array( __CLASS__, 'load_script_func' ), 10, 1);
	}

	public static function jvfrm_spot_inline_category_slider_callback( $atts, $content='' )
	{
		...
		$jvfrm_spot_getUserQuery	= new WP_User_Query( $arrGetUserArgs );
		$jvfrm_spot_getUsers			= $jvfrm_spot_getUserQuery->get_results();
		ob_start();
		...
		<div id="javo-inline-category-slider-wrap">
			<div id="javo-inline-category-slider-inner">
				<div id="javo-inline-category-slider" class="owl-carousel owl-theme" style="display:block;">
					...
					<?php
					if( !empty( $jvfrm_spot_getUsers ) ) : foreach( $jvfrm_spot_getUsers as $user ) {
							$user_image = get_avatar( $user->ID, 150  );
						?>

						<div class="item javo-inline-category">
							<a href="<?php echo function_exists( 'jvfrm_spot_getUserPage') ? jvfrm_spot_getUserPage( $user->ID ) : '#';?>">
								<?php
								if( $user_image!='' ){
									echo $user_image;
								}else{
									printf('<img src="%s" class="img-responsive wp-post-image" style="margin: 0 auto; width:121px; height:121px; border-radius:%s%%;">', $jvfrm_spot_tso->get('no_image', JVFRM_SPOT_IMG_DIR.'/no-image.png')
									, $radius);
								};?>
