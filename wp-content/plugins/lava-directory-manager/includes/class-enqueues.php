...
class Lava_Directory_Manager_Enqueues extends Lava_Directory_Manager
{
	...
	public function register_scripts()
	{
		...
		wp_enqueue_script(
			'google-maps'
			, "{$this->lava_ssl}maps.googleapis.com/maps/api/js?libraries=places"
			, Array('jquery')
			, "0.0.1"
			, false
		);
		...
		wp_enqueue_script( 'lava-directory-manager-less-min-js' );
	}
	...
	public function admin_styles() {
		wp_enqueue_style(
			...
			, lava_get_directory_manager_assets_url() . "css/admin.css"
			, false
			, '1.0.0'
		);
	}
